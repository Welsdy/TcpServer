...
int event_loop_run(struct event_loop *eventLoop){
    ...
    while (!eventLoop->quit){
        dispatcher->dispatch(eventLoop,&time);
...
void event_loop_channel_buffer_nolock(struct event_loop *eventLoop, int fd, struct channel *channel1, int type){
    struct channel_element *channelElement = malloc(sizeof(struct channel_element));//add channel into the pending list
...
int handleWakeup(void *data){
    ...
    if(n != sizeof(one)){
        error(1,errno,"handleWakeup  failed");
    }
    printf("wakeup, %s\n", eventLoop->thread_name);
}
...
int event_loop_do_channel_event(struct event_loop *eventLoop, int fd, struct channel *channel1, int type){
    pthread_mutex_lock(&eventLoop->mutex);// get lock
    assert(eventLoop->is_handle_pending == 0);

    // 往该线程的 channel 列表里增加新的 channel
    event_loop_channel_buffer_nolock(eventLoop,fd,channel1,type);

    pthread_mutex_unlock(&eventLoop->mutex); //release the lock
